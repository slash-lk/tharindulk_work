...
namespace MultitabledDataSetApp
{
    public partial class MainForm : Form
    {
        ...
        private void BuildTableRelationship()
        {
            ...
            dr = new DataRelation("InventoryOrder", _autoLotDs.Tables["Inventory"].Columns["CarID"],
        ...
        private void btnGetOrderDetails_Click(object sender, EventArgs e)
        {
            ...
            DataRow[] drsCust = _autoLotDs.Tables["Customers"].Select($"CustID = {custID}");

            strOrderInfo += $"Customer {drsCust[0]["CustID"]}: {drsCust[0]["FirstName"].ToString().Trim()} " +
                $"{drsCust[0]["LastName"].ToString().Trim()}\n";

            // Navigate from customer table to order table
            DataRow[] drsOrder = drsCust[0].GetChildRows(_autoLotDs.Relations["CustomerOrder"]);
